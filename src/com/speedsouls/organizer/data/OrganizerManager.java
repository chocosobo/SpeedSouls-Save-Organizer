...
public class OrganizerManager
{
	...
	static
	{
		...
		{
			JOptionPane.showMessageDialog(null, "실행 중 데이터를 읽어들이지 못 했습니다. 정리기를 시작하지 못 했습니다.",
	...
	private static void initKeyboardHook()
	{
		...
		{
			JOptionPane.showMessageDialog(mainWindow, "키보드 후크 중 문제가 생겼습니다. 글로벌 단축키가 비활성화 되었습니다.",
					"에러 발생", JOptionPane.ERROR_MESSAGE);
	...
	public static void importAsProfiles(File[] files, Game game)
	{
		...
		for (File file : files)
		{
			...
			{
				JOptionPane.showMessageDialog(mainWindow, "프로파일을 불러오는데 실패했습니다!", "에러 발생",
	...
	public static void createFolder(String name)
	{
		...
		if (dir.exists())
		{
			JOptionPane.showMessageDialog(mainWindow, "이 폴더는 이미 존재합니다!", "에러 발생", JOptionPane.ERROR_MESSAGE);
			return;
		}
		dir.mkdirs();
	...
	private static File createFileForNewSave(Folder parent)
	{
		if (getSelectedGame().getSaveFileLocation() == null)
		{
			JOptionPane.showMessageDialog(mainWindow,
					"세이브 파일을 불러오려면 프로파일에 위치를 설정해야합니다!", "에러 발생",
		...
		{
			JOptionPane.showMessageDialog(mainWindow, "세이브 파일을 불러오던 중 에러가 발생했습니다!", "에러 발생",
					JOptionPane.ERROR_MESSAGE);
		}
		return newFile;
	}
	...
	public static void openWebPage()
	{
		...
		{
			JOptionPane.showMessageDialog(mainWindow, e.getMessage(), "에러 발생", JOptionPane.ERROR_MESSAGE);
		}
	}
